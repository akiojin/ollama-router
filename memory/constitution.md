...
- **明確な責任分離**:
  - **Registry**: エージェント情報の保存・管理
  - **API**: HTTP エンドポイント処理
  - **Proxy**: リクエストのルーティング
  - **Monitor**: ヘルスチェックとメトリクス収集
- **JSONファイルベースのストレージ**: データベースの複雑さを避け、`~/.ollama-coordinator/agents.json` を使用
...
**ヘルスチェック:**
- **Heartbeat-based**: エージェントが定期的にハートビート送信
- **パッシブモニタリング**: コーディネーターからのポーリングなし
...
### エージェント管理
- **自己登録**: エージェントは起動時に自動登録
- **Heartbeat**: 定期的なハートビート送信でオンライン状態を維持
- **自動クリーンアップ**: タイムアウトしたエージェントは自動的にオフライン化

### APIプロキシ
- **透過的**: クライアントは単一エンドポイントにアクセス
- **ステートレス**: プロキシ自体は状態を持たない
- **フェイルオーバー**: エージェント障害時は別のエージェントにリトライ

### パフォーマンス目標
- **レスポンスタイム**: p95 < 100ms（プロキシオーバーヘッド）
- **スループット**: 1000 req/s（単一コーディネーター）
- **メモリ使用量**: < 50MB（アイドル時）
- **起動時間**: < 1秒
