...
/// # Returns
/// * `Ok(User)` - 作成されたユーザー
/// * `Err(RouterError)` - 作成失敗（ユーザー名重複など）
pub async fn create(
    pool: &SqlitePool,
    username: &str,
    password_hash: &str,
    role: UserRole,
) -> Result<User, RouterError> {
...
pub async fn create_with_id(
    ...
    .map_err(|e| {
        if e.to_string().contains("UNIQUE constraint failed") {
            RouterError::Database(format!("Username '{}' already exists", username))
        } else {
            RouterError::Database(format!("Failed to create user: {}", e))
        }
    })?;
...
pub async fn find_by_username(
    pool: &SqlitePool,
    username: &str,
) -> Result<Option<User>, RouterError> {
    ...
    .fetch_optional(pool)
    .await
    .map_err(|e| RouterError::Database(format!("Failed to find user: {}", e)))?;
...
pub async fn list(pool: &SqlitePool) -> Result<Vec<User>, RouterError> {
    let rows = sqlx::query_as::<_, UserRow>(
        "SELECT id, username, password_hash, role, created_at, last_login FROM users ORDER BY created_at DESC"
    )
    .fetch_all(pool)
    .await
    .map_err(|e| RouterError::Database(format!("Failed to list users: {}", e)))?;
...
pub async fn update(
    ...
    let current = find_by_id(pool, id)
        .await?
        .ok_or_else(|| RouterError::Database(format!("User not found: {}", id)))?;
    ...
    sqlx::query("UPDATE users SET username = ?, password_hash = ?, role = ? WHERE id = ?")
        ...
        .execute(pool)
        .await
        .map_err(|e| RouterError::Database(format!("Failed to update user: {}", e)))?;
...
/// * `Ok(())` - 更新成功
/// * `Err(RouterError)` - 更新失敗
pub async fn update_last_login(pool: &SqlitePool, id: Uuid) -> Result<(), RouterError> {
    let now = Utc::now();
    ...
    sqlx::query("UPDATE users SET last_login = ? WHERE id = ?")
        ...
        .execute(pool)
        .await
        .map_err(|e| RouterError::Database(format!("Failed to update last login: {}", e)))?;
...
pub async fn delete(pool: &SqlitePool, id: Uuid) -> Result<(), RouterError> {
    sqlx::query("DELETE FROM users WHERE id = ?")
        ...
        .execute(pool)
        .await
        .map_err(|e| RouterError::Database(format!("Failed to delete user: {}", e)))?;
...
/// * `Ok(None)` - ユーザーが見つからなかった
/// * `Err(RouterError)` - 検索失敗
pub async fn find_by_id(pool: &SqlitePool, id: Uuid) -> Result<Option<User>, RouterError> {
    let row = sqlx::query_as::<_, UserRow>(
...
/// * `Ok(false)` - ユーザーが存在する
/// * `Err(RouterError)` - チェック失敗
pub async fn is_first_boot(pool: &SqlitePool) -> Result<bool, RouterError> {
    let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM users")
        .fetch_one(pool)
        .await
        .map_err(|e| RouterError::Database(format!("Failed to check first boot: {}", e)))?;
...
/// * `Err(RouterError)` - チェック失敗
pub async fn is_last_admin(pool: &SqlitePool, user_id: Uuid) -> Result<bool, RouterError> {
    // 対象ユーザーが管理者かチェック
    let user = find_by_id(pool, user_id)
        .await?
        .ok_or_else(|| RouterError::Database(format!("User not found: {}", user_id)))?;
    ...
    let admin_count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM users WHERE role = 'admin'")
        .fetch_one(pool)
        .await
        .map_err(|e| RouterError::Database(format!("Failed to count admins: {}", e)))?;
