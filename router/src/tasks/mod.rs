...
impl DownloadTaskManager {
    ...
    /// タスクを作成して登録
    pub async fn create_task(&self, node_id: Uuid, model_name: String) -> DownloadTask {
        let task = DownloadTask::new(node_id, model_name);
        let task_id = task.id;
    ...
    /// 特定のノードのタスクを取得
    pub async fn list_tasks_by_agent(&self, node_id: Uuid) -> Vec<DownloadTask> {
        let tasks = self.tasks.lock().await;
        tasks
            .values()
...
mod tests {
    ...
    async fn test_update_progress() {
        ...
        let task = manager.create_task(node_id, "test-model".to_string()).await;
    ...
    async fn test_cleanup_finished_tasks() {
        ...
        let task1 = manager.create_task(node_id, "model1".to_string()).await;
        let task2 = manager.create_task(node_id, "model2".to_string()).await;
        let _task3 = manager.create_task(node_id, "model3".to_string()).await;
