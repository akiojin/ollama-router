...
2. まずノードローカル `~/.llm-router/models/<name>/model.gguf` を探す。あれば採用。
3. ルーターから受け取った `path` が存在し読み取り可能なら、それを直接使用（コピーせず可）。
4. `path` が不可でも `download_url` があればそれをダウンロードし、`~/.llm-router/models` に保存。
5. いずれも不可ならエラーを返す。Ollama blob 等への暗黙フォールバックは禁止。
