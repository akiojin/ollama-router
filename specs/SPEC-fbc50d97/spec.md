...
**入力**: ユーザー説明: "コーディネーターに送られたリクエストとエージェントから
...
オペレーターとして、ダッシュボードでどのリクエストがどのエージェントで処理されたかを
時系列で確認したい。システムがどのエージェントにリクエストを振り分けているか、
...
各レコードに時刻・エージェント名・モデル名・処理時間・成功/失敗ステータスが含まれる
ことを確認する。

**受け入れシナリオ**:

1. **前提** コーディネーターが複数のリクエストを処理済み、**実行** オペレーターが
   ダッシュボードの履歴タブを開く、**結果** 時系列順にリクエスト一覧が表示される
2. **前提** リクエストが成功と失敗の両方を含む、**実行** 履歴一覧を確認、**結果**
   各リクエストのステータスが視覚的に区別できる（例：色分け）
3. **前提** エージェントが複数存在する、**実行** 履歴を確認、**結果** どのエージェント
...
管理者として、特定のモデル名、エージェント、成功/失敗ステータス、日時範囲で
...
**独立テスト**: フィルタ条件（例：エージェント名="agent-001"、ステータス="エラー"）を
...
2. **前提** 複数のエージェントが存在、**実行** 特定のエージェントでフィルタ、
   **結果** そのエージェントが処理したリクエストのみが表示される
...
- **FR-001**: システムはコーディネーターが受信したすべてのリクエスト（Chat、
  Generate）の本文を記録する必要がある
- **FR-002**: システムはエージェントから返されたすべてのレスポンスの本文を記録
  する必要がある（エラー時はエラー内容を記録）
- **FR-003**: システムは各リクエスト/レスポンスのメタデータ（タイムスタンプ、
  エージェントID、モデル名、処理時間、成功/失敗ステータス）を記録する必要がある
...
- **FR-007**: ダッシュボードはモデル名、エージェント、成功/失敗ステータス、
...
- 統計グラフの追加（モデル別の使用率、エージェント別の負荷分散状況等）
...
- 保存方式はJSONファイルベースとし、SQLiteやPostgreSQL等のRDBMSは使用しない
  （既存のエージェント情報保存方式と統一）
- 保存場所は `~/.ollama-coordinator/request_history.json` とし、環境変数
  `OLLAMA_COORDINATOR_DATA_DIR` で変更可能とする
...
- コーディネーターサーバーが既に稼働しており、エージェントが登録されている
...
- コーディネーターのプロキシAPI機能（`/api/chat`, `/api/generate`）
...
3. 管理者は特定のフィルタ条件（例：エージェント名）で履歴を3秒以内に絞り込める
