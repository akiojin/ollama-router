...
2. **前提** 開発ブランチへのマージ完了、**実行** マージ後の自動処理、
   **結果** アルファ版番号（例: v1.2.3-alpha.1）でリリースが自動作成される
3. **前提** アルファ版リリース作成、**実行** リリースページを確認、
...
### ユーザーストーリー3 - 緊急修正のリリース (優先度: P2)

開発者として、本番で重大なバグを発見したら緊急修正コマンドで
...
**独立テスト**: 緊急修正コマンドを実行して修正ブランチを作成し、
修正後に本番ブランチへのプルリクエストを作成して、品質チェック合格後に
パッチバージョンでリリースされることを確認できる。

**受け入れシナリオ**:

1. **前提** 本番環境で問題を発見、**実行** 緊急修正コマンド実行、
   **結果** 緊急修正用ブランチが作成され、修正ガイドが表示される
...
- **FR-004**: システムは開発ブランチへのマージ時にアルファ版リリースを自動作成する必要がある
...
- **FR-009**: ユーザーはコマンド一つで緊急修正プロセスを開始できる必要がある
...
- **バージョン番号**: システムのリリースを識別する番号。
  メジャー・マイナー・パッチの3つの数値と、アルファ版の場合は追加の識別子で構成される
...
1. **開発者は機能開発完了後、5分以内にアルファ版リリースを確認できる**
