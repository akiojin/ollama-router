...
- **FR-016**: エージェントはローカルOllamaインスタンスの稼働状況を監視する必要がある
- **FR-017**: エージェントは定期的にコーディネーターにヘルスチェック情報（CPU使用率、メモリ使用率、処理中リクエスト数）を送信する必要がある
- **FR-018**: エージェントはコーディネーターからのリクエストをローカルOllamaインスタンスに転送し、レスポンスを返す必要がある
- **FR-019**: エージェントはWindows起動時に自動起動できる必要がある（オプション設定可能）
- **FR-020**: エージェントはコーディネーターと同一マシン上で動作可能である必要がある
...
- Ollama 0.1.0以降がローカルにインストールされている必要がある
...
- 各マシンにOllamaが既にインストールされており、HTTP API経由でアクセス可能である
...
- **Ollama**: ローカルLLMランタイム。各エージェントマシン上で動作し、HTTP API（デフォルト: `http://localhost:11434`）を提供する必要がある。
...
1. **エージェント登録の即座性**: エージェントが起動してからコーディネーターに登録されるまでの時間が5秒以内である
2. **リクエスト振り分けの均等性**: 10個のリクエストを送信した場合、3台のエージェントがそれぞれ2〜5個のリクエストを処理する（完全に均等ではないが大きな偏りがない）
3. **障害検知の迅速性**: エージェントがオフラインになってから60秒以内にコーディネーターが検知し、リクエスト振り分けから除外する
4. **ダッシュボードのリアルタイム性**: エージェントの状態変化（オンライン↔オフライン）がダッシュボード上で10秒以内に反映される
5. **システムの安定性**: コーディネーターが24時間連続稼働し、エージェントの登録・解除、リクエスト処理を問題なく継続できる
6. **ユーザビリティ**: 技術者でないユーザーが、設定ウィンドウでコーディネーターURLを入力するだけでエージェントを登録できる
7. **同一マシン動作**: コーディネーターとエージェントが同一マシン上で動作し、localhostとして正常に認識・処理される
