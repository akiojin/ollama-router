...
1. **前提** エージェント一覧が表示されている、**実行** あるエージェントの「設定」ボタンをクリックする、**結果** エージェント設定画面が開き、エージェント名、メモ、タグを編集できる
2. **前提** エージェント設定画面が開いている、**実行** エージェント名を変更して「保存」をクリックする、**結果** 変更が即座に反映され、エージェント一覧に新しい名前が表示される
3. **前提** 不要なエージェントが登録されている、**実行** エージェント一覧で「削除」ボタンをクリックし、確認ダイアログで「削除」を選択する、**結果** エージェントが登録解除され、一覧から削除される
4. **前提** 問題のあるエージェントが登録されている、**実行** エージェント一覧で「強制切断」ボタンをクリックする、**結果** エージェントが即座にOffline状態になる
...
ダッシュボードからエージェントを制御できる。

- 制御機能:
  - エージェントの登録解除（削除）
  - エージェントの強制切断（Offline化）
  - 将来的な拡張：
    - エージェントの起動/停止コマンド送信（OS依存）
    - エージェント設定のリモート更新
- 確認ダイアログ:
  - 削除時に確認ダイアログ表示
  - 誤操作防止
