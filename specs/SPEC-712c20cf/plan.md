...
**主要要件**:
- エージェント一覧表示（マシン名、IP、ステータス、稼働時間）
- リアルタイム更新（5秒ごとのポーリング）
- パフォーマンスメトリクス可視化（CPU、メモリ、リクエスト数）
- システム統計表示（総エージェント数、オンライン数、総リクエスト数）
...
2. 5秒間隔なら負荷も許容範囲（1000エージェント × 5秒 = 200 req/min）
...
1. **シナリオ1**: ダッシュボードアクセス
   - 前提: コーディネーターが起動
   - 実行: `GET /dashboard`
   - 結果: HTMLページが返却され、エージェント一覧が表示

2. **シナリオ2**: エージェント情報表示
   - 前提: 複数のエージェントが登録
   - 実行: `GET /api/dashboard/agents`
   - 結果: JSON配列で全エージェント情報が返却
