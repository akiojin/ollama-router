...
1. **前提** ダッシュボードでモデル管理画面を表示、**実行** 「ダウンロード可能なモデル」タブを選択、**結果** LLM runtime公式ライブラリから取得したモデル一覧（名前、サイズ、説明）が表示される
...
- LLM runtime公式ライブラリAPIがアクセス不能な場合、どうなるか？
  - 既存ノードの `runtime list` からモデル一覧を取得し、縮退運転モードで動作する
...
- **FR-001**: システムはノード登録時に、対応モデル4件を自動ダウンロードする必要がある
...
- **FR-005**: システムはLLM runtime公式ライブラリからダウンロード可能なモデル一覧を取得して表示する必要がある
...
- **FR-011**: システムはリクエスト受信時に必要なモデルをオンデマンドでロードする必要がある
- **FR-012**: システムは一定時間アイドル状態のモデルを自動的にアンロードしてメモリを解放する必要がある
- **FR-013**: システムはGPUメモリ容量に応じて同時ロード可能なモデル数を制限する必要がある

### 主要エンティティ

- **モデル情報**: LLM runtimeモデルの名前、サイズ、説明、必要メモリを表す
...
- LLM runtime公式ライブラリAPIへのアクセスにはインターネット接続が必要
...
- ノード側にLLM runtimeがインストールされている、または自動インストール機能が有効である
...
メモリ効率を最大化するため、モデルは事前ロードではなくオンデマンドでロード・アンロードする。

### 動作仕様

1. **オンデマンドロード**: リクエスト受信時に必要なモデルがロードされていなければ、その時点でロードする
2. **アイドルアンロード**: 一定時間（デフォルト: 5分）使用されていないモデルは自動的にアンロードしてメモリを解放する
3. **事前ダウンロード**: モデルファイル自体はノード登録時にダウンロードしておく（ロード時間短縮のため）

### メモリ管理

- 同時にロードできるモデル数はノードのGPUメモリに依存する
- 新しいモデルのロード時にメモリ不足の場合、最も長くアイドル状態のモデルをアンロードする
- ロード中のリクエストは待機状態となり、ロード完了後に処理される

### 設定項目

- `LLM_MODEL_IDLE_TIMEOUT`: アイドルタイムアウト秒数（デフォルト: 300）
- `LLM_MAX_LOADED_MODELS`: 同時ロード可能な最大モデル数（デフォルト: 自動計算）

---

## アーキテクチャ要件（通信経路）

- ルーターはノードの「OpenAI互換API（単一ポート、標準は LLM runtime ポート+1）」のみと通信する。  
  - 登録ヘルスチェック、モデル自動配布指示、推論プロキシ（chat/completions/embeddings）はすべてノードAPI経由。  
  - ルーターがノード内部の LLM runtime を直接叩かないこと。  
- ノードは受けた OpenAI互換リクエストを内部の LLM runtime にルーティングし、必要なモデルを ensure/pull してから実行する。  
- 複数 LLM runtime 実体を持つ場合の振り分けはノード側の責務とし、ルーターは単一 API エンドポイントだけを認識すればよい。  
- `/v1/models` 応答はルーターが定義する対応モデル固定リストを返す（ノード側で揃える）。

### モデルダウンロード完了までの受付制御
- 対応モデル4件のダウンロードがすべて完了するまではノードは `initializing` 状態とし、ロードバランサの選択対象外とする。
- ダウンロード完了後は `ready` 状態となり、リクエスト受付可能となる（モデルはオンデマンドでロードされる）。
- ルーターからの推論リクエストは、全ノードが `initializing` の場合は 503（準備中）を返す。
- ハートビートで `initializing` フラグと `downloaded_models`（ダウンロード済み/総数）を送信する。
- ノードAPI `/v1/models` でダウンロード進捗情報を返せること（任意だが推奨）。
...
9. ノード登録完了後、対応モデル4件（gpt-oss:120b/20b、gpt-oss-safeguard:20b、qwen3-coder:30b）が当該ノードで自動ダウンロードされること（ダッシュボードまたは API で確認可能であること）
10. モデルはオンデマンドでロードされ、アイドルタイムアウト後に自動アンロードされること
11. 同時に複数モデルをロードしてもGPUメモリを超過しないこと
