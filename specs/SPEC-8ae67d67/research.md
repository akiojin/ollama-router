# リサーチドキュメント: コーディネーター主導のモデル自動配布機能
...
**主要アプローチ**:
1. **エージェント経由でモデル一覧を取得**:
   - 各エージェントの `GET /api/tags` エンドポイントを呼び出し
...
**代替案が却下された理由**:
- WebSocket: コーディネーターとエージェント間の通信はREST APIベースであり、WebSocketは設計の一貫性を損なう
...
2. **非同期実行**: `tokio::spawn` でエージェントへのHTTPリクエストを実行
...
**コーディネーター側での取得**:
- エージェント登録時に `RegisterRequest` から `gpu_memory_total_mb` を取得（現在は送信されていないため、追加が必要）
...
- すべてのエージェントに同じモデル: GPU能力の無駄、ディスク容量の無駄
...
| コンポーネント | 技術選択 | 理由 |
|---------------|---------|------|
| モデル一覧取得 | エージェント経由 + 事前定義リスト | 公式API不在のため |
