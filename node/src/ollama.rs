...
impl OllamaManager {
    ...
    pub async fn ensure_running(&mut self) -> NodeResult<()> {
    ...
    async fn ensure_default_model(&self) -> NodeResult<()> {
    ...
    /// 指定モデルが存在しなければプルする
    pub async fn ensure_model(&self, model: &str) -> NodeResult<()> {
        let name = model.trim();
        if name.is_empty() {
            return Ok(());
        }
        ...
        if self.model_exists(name).await? {
            ...
            Err(NodeError::OllamaConnection(format!(
    ...
    /// モデル存在チェック
    async fn model_exists(&self, model: &str) -> NodeResult<bool> {
        let name = model.trim();
        if name.is_empty() {
            return Ok(true);
        }
    ...
    pub async fn remove_model(&self, model: &str) -> NodeResult<()> {
    ...
    /// モデル一覧を取得
    pub async fn list_models(&self) -> NodeResult<Vec<String>> {
    ...
    async fn fetch_models(&self, fallback: bool) -> NodeResult<Vec<String>> {
        ...
        let response = client.get(&url).send().await.map_err(|e| {
            NodeError::OllamaConnection(format!("Failed to query Ollama tags: {}", e))
        })?;

        if !response.status().is_success() {
            return Err(NodeError::OllamaConnection(format!(
        ...
        let payload = response
            .json::<TagsResponse>()
            .await
            .map_err(|e| NodeError::Internal(format!("Failed to parse tags response: {}", e)))?;
    ...
    /// モデルをプル（リトライ付き・進捗表示）
    async fn pull_model(&self, model: &str) -> NodeResult<()> {
        ...
        .await
        .map_err(|e| {
            NodeError::OllamaConnection(format!(
        ...
        while let Some(chunk) = stream.next().await {
            let chunk = chunk.map_err(|e| {
                NodeError::Internal(format!("Failed to read pull stream for {}: {}", model, e))
            })?;
    ...
    /// Ollamaをダウンロード
    async fn download(&self) -> NodeResult<()> {
        ...
        if let Some(parent) = self.ollama_path.parent() {
            std::fs::create_dir_all(parent)
                .map_err(|e| NodeError::Internal(format!("Failed to create directory: {}", e)))?;
        }
        ...
        while let Some(chunk) = stream.next().await {
            let chunk =
                chunk.map_err(|e| NodeError::Internal(format!("Failed to read chunk: {}", e)))?;
            buffer.extend_from_slice(&chunk);
    ...
    fn start(&mut self) -> NodeResult<()> {
        ...
        let child = command
            ...
            .spawn()
            .map_err(|e| NodeError::OllamaConnection(format!("Failed to start Ollama: {}", e)))?;
    ...
    /// Ollama起動を待つ
    async fn wait_for_startup(&mut self) -> NodeResult<()> {
    ...
    /// Ollamaのバージョンを取得
    pub async fn get_version(&self) -> NodeResult<String> {
        let output = Command::new(&self.ollama_path)
            .arg("--version")
            .output()
            .map_err(|e| {
                NodeError::OllamaConnection(format!("Failed to get Ollama version: {}", e))
            })?;

        if !output.status.success() {
            return Err(NodeError::OllamaConnection(
    ...
    pub fn stop(&mut self) -> NodeResult<()> {
        if let Some(mut process) = self.process.take() {
            process.kill().map_err(|e| {
                NodeError::Internal(format!("Failed to kill Ollama process: {}", e))
            })?;
...
fn save_ollama_binary(bytes: &[u8], download_url: &str, destination: &Path) -> NodeResult<()> {
...
fn extract_tar_gz(bytes: &[u8], destination: &Path) -> NodeResult<()> {
    ...
    for entry in archive
        .entries()
        .map_err(|e| NodeError::Internal(format!("Failed to read archive: {}", e)))?
    {
        let mut entry =
            entry.map_err(|e| NodeError::Internal(format!("Failed to read entry: {}", e)))?;
        ...
        let path = entry
            .path()
            .map_err(|e| NodeError::Internal(format!("Failed to read entry path: {}", e)))?;
        if path
            .file_name()
            ...
            .unwrap_or(false)
        {
            let mut file = File::create(destination)
                .map_err(|e| NodeError::Internal(format!("Failed to create file: {}", e)))?;
            std::io::copy(&mut entry, &mut file)
                .map_err(|e| NodeError::Internal(format!("Failed to extract file: {}", e)))?;
...
fn extract_zip(bytes: &[u8], destination: &Path) -> NodeResult<()> {
    let cursor = Cursor::new(bytes);
    let mut archive = ZipArchive::new(cursor)
        .map_err(|e| NodeError::Internal(format!("Failed to read zip archive: {}", e)))?;
    ...
    for i in 0..archive.len() {
        let mut file = archive
            .by_index(i)
            .map_err(|e| NodeError::Internal(format!("Failed to access zip entry: {}", e)))?;
        if !file.is_file() {
            continue;
        }
...
fn write_binary(bytes: &[u8], destination: &Path) -> NodeResult<()> {
    std::fs::write(destination, bytes)
        .map_err(|e| NodeError::Internal(format!("Failed to write Ollama binary: {}", e)))?;
    set_unix_executable(destination)?;
    Ok(())
}

fn set_unix_executable(_path: &Path) -> NodeResult<()> {
    #[cfg(unix)]
    {
        use std::os::unix::fs::PermissionsExt;
        let metadata = std::fs::metadata(_path)
            .map_err(|e| NodeError::Internal(format!("Failed to get file metadata: {}", e)))?;
        let mut perms = metadata.permissions();
        perms.set_mode(0o755);
        std::fs::set_permissions(_path, perms)
            .map_err(|e| NodeError::Internal(format!("Failed to set execute permission: {}", e)))?;
    }
    Ok(())
}
...
fn get_ollama_directory() -> PathBuf {
    if cfg!(windows) {
        // Windows: %LOCALAPPDATA%\Or\ollama
        let local_app_data = std::env::var("LOCALAPPDATA")
            .unwrap_or_else(|_| String::from("C:\\Users\\Default\\AppData\\Local"));
        PathBuf::from(local_app_data).join("Or").join("ollama")
    } else if cfg!(target_os = "macos") {
        // macOS: ~/Library/Application Support/Or/ollama
...
fn verify_checksum(data: &[u8], expected_checksum: &str) -> NodeResult<()> {
...
async fn fetch_checksum_from_url(
    ...
    let checksum = response
        .text()
        .await
        .map_err(|e| NodeError::Internal(format!("Failed to read checksum: {}", e)))?
        .trim()
        .to_string();
