//! Ollama Router Node Entry Point
#[cfg(any(target_os = "windows", target_os = "macos"))]
use or_node::gui::tray::{run_with_system_tray, TrayOptions};
use or_node::settings::{load_settings_from_disk, start_settings_panel, StoredSettings};
use or_node::{
...
use or_node::ollama_pool::OllamaPool;
use std::{
    io::{self, Write},
    sync::Arc,
};
...
fn main() {
    ...
    let tray_options = TrayOptions::new(&config.router_url, settings_panel.url());
...
async fn run_agent(config: LaunchConfig) -> NodeResult<()> {
    info!("Ollama Router Node v{}", env!("CARGO_PKG_VERSION"));
    ...
    let mut coordinator_client = CoordinatorClient::new(router_url.clone());
    ...
    if !gpu_devices_valid(&gpu_devices) {
        ...
        wait_for_user_ack(
            "GPU ハードウェアが検出できません。GPU を搭載したマシンでのみノードを利用できます。",
        );
    ...
    info!("Registered successfully! Node ID: {}", node_id);

    // 対応モデルを取得し、全モデルを先に確保（モデルごとに独立した Ollama インスタンスを起動）
    let model_list = fetch_models(&router_url).await.unwrap_or_default();
    {
    ...
    loop {
        ...
        let metrics = match metrics_collector.collect_metrics() {
            ...
            Err(e) => {
                ...
                or_node::metrics::SystemMetrics::placeholder()
            }
        };
        ...
        let heartbeat_req = HealthCheckRequest {
            ...
            // ノードはHTTPサーバーではないため、常に0を送信
            // 将来的にノードがHTTPサーバーとして動作する場合に実装予定
            active_requests: 0,
...
#[allow(dead_code)]
async fn fetch_coordinator_models(router_url: &str) -> NodeResult<Vec<String>> {
    let url = format!("{}/v1/models", router_url.trim_end_matches('/'));
        ...
        match client.get(&url).send().await {
            Ok(resp) => {
                if !resp.status().is_success() {
                    last_err = Some(NodeError::CoordinatorConnection(format!(
                        "list models returned HTTP {}",
                        resp.status()
                    )));
                } else {
                    let body: serde_json::Value = resp.json().await.map_err(|e| {
                        NodeError::Internal(format!("Failed to parse models response: {}", e))
                    })?;
    ...
    Err(last_err.unwrap_or_else(|| {
        NodeError::CoordinatorConnection("list models failed without details".to_string())
    }))
}
...
impl LaunchConfig {
    fn from_env_or_settings(stored: &StoredSettings) -> Self {
        let router_url = std::env::var("ROUTER_URL")
            .ok()
            .or_else(|| stored.router_url())
...
async fn send_heartbeat_once(
    ...
    gpu_capability: &Option<or_node::metrics::GpuCapability>,
...
async fn register_with_retry(
    ...
    req: RegisterRequest,
) -> NodeResult<RegisterResponse> {
    let retry_interval = registration_retry_interval();
    let max_attempts = registration_retry_limit();
    let mut attempts = 0usize;
    let router_url = client.router_url().to_string();
    const GPU_HELP_MESSAGE: &str = r#"
========================================
ERROR: GPU Required
========================================
This coordinator requires nodes to have GPU available.
        ...
        match client.register(req.clone()).await {
            ...
            Err(err) => {
                ...
                warn!(
                    ...
                    router_url, attempts, target, err
                );
...
mod tests {
    use super::*;
    use ollama_router_common::protocol::RegisterStatus;
