...
/// ルーターの /v1/models を取得（リトライ付き）
#[allow(dead_code)]
pub async fn fetch_models(router_url: &str) -> NodeResult<Vec<String>> {
...
/// ルーターが返す全モデルを確保（ベストエフォート）。
/// 失敗しても他モデルの確保は続行し、ログに残す。
#[allow(dead_code)]
pub async fn sync_all_models(router_url: &str, ollama_manager: &mut OllamaManager) {
    let models = match fetch_models(router_url).await {
        Ok(m) => m,
        Err(e) => {
...
mod tests {
    ...
    impl DummyEnsure {
        async fn ensure_model(&self, model: &str) -> NodeResult<()> {
