...
- GPU未搭載ノードは登録させない。API・UI・テストでGPUデバイス情報を必須とする。
...
ルーター主導のAIモデル自動配布システムを提供します。

#### 機能概要

- **自動配布**: ノード登録時にGPUメモリに応じて最適なモデルを自動選択・配布
- **手動配布**: 管理者が特定/全ノードに任意のモデルを配布可能
- **進捗追跡**: リアルタイムでダウンロード進捗を監視・表示
- **モデル管理**: 利用可能/インストール済みモデルの一覧表示
...
- `GET /api/models/available` - 利用可能なモデル一覧
- `POST /api/models/distribute` - モデルを配布（特定/全ノード）
- `POST /api/agents/{id}/models/pull` - 特定ノードにモデルプル指示
- `GET /api/agents/{id}/models` - ノードのインストール済みモデル
- `GET /api/tasks/{id}` - タスク進捗取得
- `POST /api/tasks/{id}/progress` - タスク進捗更新（ノード→ルーター）

#### エラーハンドリング

- **オフラインノード**: 503 Service Unavailable
- **無効なモデル名**: 400 Bad Request
- **ストレージ容量不足**: 507 Insufficient Storage

#### 詳細ドキュメント

- 機能仕様: `specs/SPEC-8ae67d67/spec.md`
- 実装計画: `specs/SPEC-8ae67d67/plan.md`
- クイックスタート: `specs/SPEC-8ae67d67/quickstart.md`
