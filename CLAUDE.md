...
コーディネーター主導のAIモデル自動配布システムを提供します。

#### 機能概要

- **自動配布**: エージェント登録時にGPUメモリに応じて最適なモデルを自動選択・配布
- **手動配布**: 管理者が特定/全エージェントに任意のモデルを配布可能
...
- `POST /api/models/distribute` - モデルを配布（特定/全エージェント）
- `POST /api/agents/{id}/models/pull` - 特定エージェントにモデルプル指示
- `GET /api/agents/{id}/models` - エージェントのインストール済みモデル
- `GET /api/tasks/{id}` - タスク進捗取得
- `POST /api/tasks/{id}/progress` - タスク進捗更新（エージェント→コーディネーター）

#### エラーハンドリング

- **オフラインエージェント**: 503 Service Unavailable
