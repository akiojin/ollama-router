...
pub async fn create_admin_interactive(pool: &sqlx::SqlitePool) -> Result<String, CoordinatorError> {
    ...
    io::stdin()
        ...
        .map_err(|e| CoordinatorError::Internal(format!("Failed to read username: {}", e)))?;
    ...
    io::stdin()
        ...
        .map_err(|e| CoordinatorError::Internal(format!("Failed to read password: {}", e)))?;
...
/// # Returns
/// * `Ok(())` - 処理成功
/// * `Err(CoordinatorError)` - 処理失敗
pub async fn ensure_admin_exists(pool: &sqlx::SqlitePool) -> Result<(), CoordinatorError> {
    // 初回起動かチェック
