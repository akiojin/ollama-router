...
async fn test_select_agent_by_metrics_prefers_low_load() {
    ...
    // エージェント登録
    // let agent1_id = registry.register(create_test_agent_request("agent1", "192.168.10.1")).await.unwrap().agent_id;
    // let agent2_id = registry.register(create_test_agent_request("agent2", "192.168.10.2")).await.unwrap().agent_id;
    // let agent3_id = registry.register(create_test_agent_request("agent3", "192.168.10.3")).await.unwrap().agent_id;
...
async fn test_fallback_to_round_robin_when_all_agents_high_load() {
    ...
    // エージェント登録
    // let agent1_id = registry.register(create_test_agent_request("agent1", "192.168.11.1")).await.unwrap().agent_id;
    // let agent2_id = registry.register(create_test_agent_request("agent2", "192.168.11.2")).await.unwrap().agent_id;
    // let agent3_id = registry.register(create_test_agent_request("agent3", "192.168.11.3")).await.unwrap().agent_id;
    ...
    // Assert: すべてのエージェントが高負荷のため、ラウンドロビンにフォールバック
    // 各エージェントが均等に選択される（9回 ÷ 3台 = 3回ずつ）
    ...
    // TODO: T014-T015で実装後にアンコメント
    panic!("RED: 全エージェント高負荷時のラウンドロビンフォールバックが未実装");
}
...
// fn create_metrics(agent_id: Uuid, cpu: f64, memory: f64, active_reqs: u32) -> MetricsUpdate {
//     use ollama_coordinator_coordinator::balancer::MetricsUpdate;
//     MetricsUpdate {
//         agent_id,
//         cpu_usage: cpu,
//         memory_usage: memory,
